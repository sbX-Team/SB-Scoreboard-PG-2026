      ...
      setTimeout(function () {
        $('#sbTakeover').velocity('transition.fadeOut', {
          ...
          complete: function () {
            ...
            if (currentLeaderboard.length > 6) {
...
function showScore (scoreboard, speed, nickname) {
  ...
  if (scoreboard.length > 0 && scoreboard[0].score === speed) {
    ipcRenderer.send('busHighScoreTrigger')
  }
  if (cycleTimer) { clearInterval(cycleTimer); cycleTimer = null }

  var formattedNickname = (nickname || '').toUpperCase().split('').join('.') + '.'
  document.getElementById('newScoreName').innerHTML = formattedNickname + "'s SCORE"
  document.getElementById('newScoreText').innerHTML = speed

  $('#newScore').velocity('transition.fadeIn', {
    duration: 800,
    complete: function () {
      ...
      setTimeout(function () {
        currentPage = 0
        renderPage(0, true)
        $('#newScore').velocity('transition.fadeOut', {
          duration: 800,
          complete: function () {
            if (cycleTimer) { clearInterval(cycleTimer); cycleTimer = null }
            if (!sbTakeoverActive && currentLeaderboard.length > 6) {
              cycleTimer = setInterval(nextPage, cycleInterval)
            }
            checkScores()
          }
...
function nextPage () {
  var pages = Math.ceil(currentLeaderboard.length / 6)
  if (pages <= 1) return
...
function renderPage (page, noAnimate) {
  ...
  var el2 = document.getElementById('leaderboard2')
  if (!el1 || !el2) return
  var start = page * 6
  var html1 = ''
  var html2 = ''
  for (var i = 0; i < 3; i++) {
    ...
    if (entry) {
      html1 += "<tr><td class='place leaderboardValue'>" + (start + i + 1) + ".</td><td class='initial leaderboardValue'>" + entry.nickname.toUpperCase().split('').join('.') + '.' + "</td><td class='score leaderboardValue'>" + entry.score + '</td></tr>'
    }
  }
  for (var i = 3; i < 6; i++) {
    var entry = currentLeaderboard[start + i]
    if (entry) {
      html2 += "<tr><td class='place leaderboardValue'>" + (start + i + 1) + ".</td><td class='initial leaderboardValue'>" + entry.nickname.toUpperCase().split('').join('.') + '.' + "</td><td class='score leaderboardValue'>" + entry.score + '</td></tr>'
    }
  }
  if (noAnimate) {
    var $t = $('#leaderboard1, #leaderboard2')
    $t.velocity('stop', true)
    el1.innerHTML = html1
    el2.innerHTML = html2
    $t.css('opacity', 1)
  } else {
    var $tables = $('#leaderboard1, #leaderboard2')
    $tables.velocity('stop').velocity({ opacity: 0 }, {
      duration: 400,
      complete: function () {
        el1.innerHTML = html1
        el2.innerHTML = html2
        $tables.velocity({ opacity: 1 }, { duration: 400 })
      }
    })
  }
}
