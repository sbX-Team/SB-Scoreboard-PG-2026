...
(function($) {
    ...
    var fi = function() {
      ...
      this.combinedCSSRegex =
        '((\\s*?@media[\\s\\S]*?){([\\s\\S]*?)}\\s*?})|(([\\s\\S]*?){([\\s\\S]*?)})'; //to match css & media queries together
    ...
    */
      ...
      for (var i = 0; i < css1.rules.length; i++) {
        ...
        rule2 = this.findCorrespondingRule(
          css2.rules,
          rule1.directive,
          rule1.value
        );
    ...
    */
    fi.prototype.intelligentCSSPush = function(
      cssObjectArray,
      minimalObject,
      reverse
    ) {
      ...
      if (cssObject === false) {
        ...
        if (minimalObject.type !== 'media') {
          for (var ii = 0; ii < minimalObject.rules.length; ii++) {
            ...
            var oldRule = this.findCorrespondingRule(
              cssObject.rules,
              rule.directive
            );
            if (oldRule === false) {
